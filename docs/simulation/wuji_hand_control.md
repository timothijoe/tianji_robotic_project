...
其 SDK 边界将一维目标 reshape 为 `(5, 4)` 后发送。本项目现在已提供仿真专用的
`SimWujiHand` 和 ROS 2 桥接，并在隔离环境中安装了官方 `wujihandpy`；但没有
打开实体设备，也没有验证电机使能、力矩上限、回零、通信频率或急停流程。
完整用法和实体首测门控见
[ROS 2 与 Wuji Hand 仿真桥接](ros2_wuji_hand_bridge.md)。
