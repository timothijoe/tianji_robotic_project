...
后保持在低刀位，左手采用两刀一个循环的 inchworm 猫爪倒手：第 1、3 刀后
左臂完全不动，由中央指垫主导回缩约 2 cm；第 2、4 刀后左臂后退约 4 cm，
同时手指平滑恢复较宽松但仍弯曲的猫爪，指垫世界坐标净后退约 2 cm。四次
逻辑倒手总计 0.08 m。低刀位倒手期间右刀必须静止，斜向换刀期间左手必须
静止。`object` 场景继续使用原有保守的逐刀左臂倒手，不使用该手指循环。

Viewer 会在轨迹预检、无关物体隐藏和双臂初始姿态设置完成后才打开。窗口首帧
直接显示正确切菜场景，并以 `GUARD_READY` 保持初始姿态后开始动作，不会先闪现
抓取任务的红/绿物体、灰色底座或通用举刀姿态。

Viewer 中蓝线是右刀规划路径，青线是右刀实际轨迹，紫线是中央 `finger3` 指垫
实际轨迹，
