...
- 左腕沿 `+Y` 移动 32 mm，分配量由四个指垫真实运动自动补偿；右刀逐样本跟踪
  最靠刀长指 pad 的 `+Y`，并在世界 `X` 跟踪四指接触区。侧向间距采用
  20 mm 优先、10 mm 回退的分级搜索，当前选择 45 mm 且满足首选层；
- 长指 MCP 不超过 0.30 rad；PIP 幅度缩小到上一版的 80%，DIP 保留 0.35 rad
  时间变化。静态中心均衡四指高度，左腕再按实际 MuJoCo 几何统一校准 Z；
...
| 指垫—刀刃侧向距离 | 45.0–45.0 mm | 搜索所得固定值 ±3 mm |
| 刀/指垫最大 X 误差 | <0.5 mm | ≤10 mm |
| 最小刀手距离 | 22 mm | 首选层 ≥20 mm |
| 最大手部桌面穿透 | 0 mm | ≤0.5 mm |
| 四指最低离桌 | 0.000–4.998 mm | 0–5 mm |
| 拇指最小离桌 | 2.98 mm | ≥2 mm |
| 指垫最差单步向刀侧波动 | 0.481 mm | ≤0.5 mm |
| 四指净后退 | 15.1–19.4 mm | 均向机器人左侧 |

活动阶段远节角保持在 20–50°。PIP 峰峰值为
`0.308 / 0.208 / 0.248 / 0.287 rad`，DIP 均为 `0.35 rad`。最新聚焦回归为
`32 passed in 96.55s`；完整仿真回归为
`261 passed, 1 warning in 468.28s`。唯一 warning 是旧 `chop` 案例已有的
39.611 N 接触力观察阈值。真实 Headless 命令返回 0 并完成 `5/5` 同步周期。
本轮实现提交为 `6b99835`。
...
- 固定侧向间距不等于 3-D 欧氏距离；刀具上下运动时完整几何距离会变化，因此两项
  指标必须继续分别验证，不能只保留侧向测试；
- MCAP 被 Git 忽略，新机器缺少默认文件时无法做真实验收；
- Viewer 当前只播放一次约 4.15 秒的同步动作，然后按 `--final-hold` 保持最终姿势；
  用户调整相机时容易错过动作。下一步应增加可配置的相机准备停留和 `--loops`，
  循环之间使用平滑复位，Headless 默认仍保持单次执行；
