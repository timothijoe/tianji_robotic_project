...
桌面手势进展：新增 `wuji-table-retreat` 离线流程，自动选择 MCAP 姿态并生成
四个长指落桌、拇指抬起、收指带动手掌后退 30 mm 的三阶段动作。真实录制选择
第 410 帧，生成 2501 帧；Headless 验收中拇指间隙 10.8 mm、最大指尖高度误差
0.39 mm、最大滑移 1.41 mm。Viewer 路径返回 0；全量回归为
`244 passed, 1 warning in 409.06s`。原始 MCAP 与 `wuji-replay` 均保持不变。
