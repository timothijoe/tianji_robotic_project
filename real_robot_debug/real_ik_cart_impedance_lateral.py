#!/usr/bin/env python3
"""Real A-arm IK + Cartesian impedance lateral down-up debug script."""
...
class MotionConfig:
    ...
    vel_ratio: int = 10
    acc_ratio: int = 10
    init_joints: tuple[float, float, float, float, float, float, float] | None = (0.0, 0.0, 0.0, -5.0, 0.0, 0.0, 0.0)
...
def parse_args(argv: list[str] | None = None) -> MotionConfig:
    ...
    parser.add_argument("--init-joints", type=parse_joints, default=(0.0, 0.0, 0.0, -5.0, 0.0, 0.0, 0.0), help="Seven comma-separated A-arm initialization joints in degrees; use 'none' to skip")
