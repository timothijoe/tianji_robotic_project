...
- SDK 兼容层把本地仿真对象包装成接近厂商 SDK 的 `create_robot()` / `create_kine()` 接口，使同一套脚本可以先跑 MuJoCo，再切到真实 SDK。
...
- **Paths**：`src/twin_control/sdk_compat.py`、`src/twin_control/sdk_kine.py`、`examples/DEMO_PYTHON_STYLE/showcase_ik_cart_impedance.py`。
- **Responsibility**：把本地 `TwinRobot`、`MarvinKinematics` 包装成接近厂商控制 SDK / 运动学 SDK 的 Python API，并提供 `mujoco` 与 `real` backend selector。
- **Input**：backend 名称、SDK root、arm/arm_type、SDK 风格 joint target、`FX_InvKineSolvePara`、mode/impedance 参数、subscribe 请求。
- **Output**：SDK 风格 robot/kine 对象、FK/IK/Jacobian 结果、SDK-shaped feedback dict。
- **Public Interface**：`create_robot()`、`create_kine()`、`create_ik_param()`、`MujocoSdkRobot`、`RealSdkRobotAdapter`、`MujocoKine`、`FX_InvKineSolvePara`。
- **Depends on**：`TwinRobot`、`MarvinKinematics`、MuJoCo runtime、厂商 `SDK_PYTHON.fx_robot` / `SDK_PYTHON.fx_kine`（real backend）。
- **Called by**：SDK-style demos、`tests/test_sdk_compat.py`、后续真实机器人调试脚本。
- **Important Semantics**：API 边界使用 SDK 单位：joint degrees、Cartesian millimetres。`MujocoSdkRobot.set_joint_position_cmd()` 是模式感知接口：POSITION/JOINT_IMPEDANCE 下作为 joint target；CARTESIAN_IMPEDANCE/FORCE 下先用 FK 把 `q_des` 转成 Cartesian target，再交给底层控制器。
...
flowchart TD
    ...
    SdkCompat[SDK Compat create_robot/create_kine]
    SdkKine[MujocoKine / FX_InvKineSolvePara]
...
- SDK 兼容层输入/输出在边界上保持厂商习惯：joint target 是 degree，FK/IK 矩阵 translation 是 millimetre；进入 `TwinRobot` / `MarvinKinematics` 后转换成 SI。
- SDK 兼容层的 Cartesian impedance joint command 会发生一次转换：`q_des(deg) -> FK(q_des) -> T_des(m)`，底层 `UnifiedController` 接收的是 Cartesian target。
...
| SDK compat shadow state | `MujocoSdkRobot.__init__()` | `set_*_state()`、`set_joint_position_cmd()`、`set_force_cmd()`、`step()`、`wait()` | compat robot 对象生命周期 | `release_robot()` / GC |
...
- `MujocoSdkRobot.subscribe()` 中的 frame serial、last command、low-speed flag、trajectory state 是本地兼容层维护或估算的影子状态，不等同于真实控制器固件原始状态。
...
### SDK-style IK + Cartesian impedance demo

1. 调用 `create_robot("mujoco", arm="B")` 创建 SDK 兼容机器人。
2. 调用 `create_kine("mujoco", arm_type=1)` 创建 SDK 兼容运动学对象。
3. 机器人切到 position，先移动到 ready joint pose。
4. 用 `MujocoKine.fk()` 计算当前 TCP pose，按 SDK 单位修改目标矩阵。
5. 用 `create_ik_param()` 创建 backend-specific IK 参数结构，再调用 `kine.ik()` 得到目标 joint pose。
6. 机器人切到 Cartesian impedance，调用 `set_joint_position_cmd()`；compat 层把 joint target 经 FK 转成 Cartesian target。
7. 循环 `step()`，通过 `subscribe()` 读取 SDK-shaped feedback。
...
- `create_robot()` / `create_kine()` / `create_ik_param()`：SDK 兼容边界，稳定性正在提高。修改 backend selector、单位、返回对象方法名会影响所有 SDK-style demos 和未来真实机器人调试脚本。
- `MujocoSdkRobot.set_joint_position_cmd()`：高影响接口。它的语义依赖当前 mode；如果改成永远 joint target，会破坏 Cartesian impedance / force demo 与厂商 SDK 的一致性。
- `FX_InvKineSolvePara`：兼容层结构体接口。字段名和 getter/setter 应尽量保持厂商风格，否则 SDK demo 迁移成本会上升。
...
7. **SDK 兼容变化**：`sdk_compat.py` / `sdk_kine.py` -> SDK-style demos -> real backend import path -> compat tests。
...
- `examples/DEMO_PYTHON_STYLE/` 用于验证 SDK-style API，不应承载底层控制逻辑。
...
- SDK 兼容层边界假设厂商 SDK 单位：joint degrees、Cartesian millimetres、rotation matrix/ABC degrees。
- `create_kine("mujoco")` 默认加载右臂切菜 MuJoCo 场景并用 MuJoCo site pose/Jacobian 做 FK/IK，不解析厂商 `.MvKDCfg`。
- Cartesian impedance 的 SDK 风格 target 假设是 joint target，经 controller/compat 层 FK 得到 Cartesian desired pose。
...
- SDK 兼容层的 `create_robot("real")` 和 `create_kine("real")` 必须能导入厂商 `SDK_PYTHON`，否则只能使用 `mujoco` backend。
- `RealSdkRobotAdapter` 必须持有有效 `DCSS()`，否则真机 `subscribe(None)` 无法保持和 MuJoCo 同样的业务调用形式。
- SDK 兼容层输出给用户的 FK/IK translation 必须是 millimetres，输出给底层控制器的 pose 必须是 metres。破坏后目标会相差 1000 倍。
...
- SDK 兼容层：要同时模拟厂商控制 SDK 与运动学 SDK，且必须维持 deg/mm 和 SI 两套单位边界。
...
- SDK 兼容层用本地估算字段填充 `subscribe()`，不能把它误认为真实控制器完整反馈。
...
- `MujocoSdkRobot` 为了贴近厂商语义会访问 `TwinRobot` 的 `_controller`、`_kinematics` 私有字段，这是临时胶水层。
- `MujocoKine.load_config()` 当前返回 vendor-shaped placeholder，不是真正的 `.MvKDCfg` parser。
...
6. 后续若真实 SDK 调试稳定，把 SDK 兼容层的私有字段访问下沉成 `TwinRobot` 正式 public API。
...
7. `src/twin_control/sdk_compat.py` 与 `src/twin_control/sdk_kine.py`：理解本地 API 如何对齐厂商 SDK。
8. `src/twin_control/controller.py` 与 `src/twin_control/kinematics.py`：理解控制和运动学的输入输出。
9. `src/twin_control/chopping.py`：看任务状态机如何使用主接口。
10. `examples/trajectory_demo.py` 与 `examples/DEMO_PYTHON_STYLE/`：看开发者如何验证原生接口和 SDK-style 接口。
11. `tests/`：最后看测试确认哪些行为是契约。
...
```text
SDK-style script
  -> create_robot/create_kine
  -> MujocoSdkRobot + MujocoKine
  -> TwinRobot + MarvinKinematics
  -> UnifiedController + MuJoCo Runtime
```
...
- `create_robot()` / `create_kine()` 是当前推荐的 SDK 迁移入口。
...
- SDK 兼容层对外遵循 degree/mm，对内转换为 rad/m。
